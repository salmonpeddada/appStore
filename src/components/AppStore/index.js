...
const initList = appsList.filter(each => each.category === 'SOCIAL')

class AppStore extends Component {
  state = {list: initList}

  onUpdated = tabId => {
    const updateList = appsList.filter(each => each.category === tabId)
    this.setState({list: updateList, isSelect: tabId})
  }
  ...
  render() {
    const {list, isSelect, searchInput} = this.state
    const searchResults = list.filter(each =>
      each.appName.toLowerCase().includes(searchInput),
    )
    const searchInputResults = searchResults.length === 0 ? list : searchResults
      ...
      <div className="bg-container">
        ...
        <ul className="bottom">
          {searchInputResults.map(each => (
            <AppItem key={each.appId} each={each} />
          ))}
        </ul>
      </div>
    )
